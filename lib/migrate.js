...
function reverseSequelizeColType (col, prefix = 'Sequelize.') {
  ...
  if (attrObj instanceof Sequelize.GEOMETRY) {
    let base = prefix + 'GEOMETRY'
    if (typeof attrObj.type !== 'undefined') {
      base += '(\'' + attrObj.type + '\''
      if (typeof attrObj.srid !== 'undefined') {
        base += ',' + attrObj.srid
      }
      base += ')'
    }
...
const reverseSequelizeDefValueType = function (defaultValue, prefix = 'Sequelize.') {
  if (defaultValue instanceof Sequelize.NOW) { return { internal: true, value: prefix + 'NOW' } }

  if (defaultValue instanceof Sequelize.UUIDV1) { return { internal: true, value: prefix + 'UUIDV1' } }

  if (defaultValue instanceof Sequelize.UUIDV4) { return { internal: true, value: prefix + 'UUIDV4' } }
...
const executeMigration = function (queryInterface, filename, pos, cb) {
  ...
  if (!mig) { return cb("Can't require file " + filename) } // eslint-disable-line
