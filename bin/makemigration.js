...
const configOptions = pathConfig(options);

if (!fs.existsSync(configOptions.modelsDir)) {
    console.log("Can't find models directory. Use `sequelize init` to create it")
    return
}

if (!fs.existsSync(configOptions.migrationsDir)) {
...
try {
    previousState = JSON.parse(fs.readFileSync(path.join(configOptions.migrationsDir, '_current.json') ));
} catch (e) { }
...
if (fs.existsSync(path.join(configOptions.migrationsDir, '_current.json')))
    fs.writeFileSync(path.join(configOptions.migrationsDir, '_current_bak.json'),
        fs.readFileSync(path.join(configOptions.migrationsDir, '_current.json'))
    );
...
currentState.revision = [
    _.padStart(date.getUTCFullYear().toString(), 4, '0'),
    _.padStart((date.getUTCMonth() + 1).toString(), 2, '0'),
...
fs.writeFileSync(path.join(configOptions.migrationsDir, '_current.json'), JSON.stringify(currentState, null, 4) );
fs.writeFileSync(path.join(configOptions.migrationsDir, '_current.json.' + currentState.revision), JSON.stringify(currentState, null, 4));
